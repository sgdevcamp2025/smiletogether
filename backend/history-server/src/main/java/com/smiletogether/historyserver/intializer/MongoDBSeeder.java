...
public class MongoDBSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // ✅ JSON 파일을 InputStream으로 로드
        InputStream jsonStream = new ClassPathResource("dummy_messages.json").getInputStream();

        // ✅ JSON → Java 객체 변환
        List<ChannelMessageDocument> rawMessages = objectMapper.readValue(jsonStream, new TypeReference<>() {});

        System.out.println("📌 JSON 파일에서 읽은 데이터 개수: " + rawMessages.size());

        // ✅ DateTimeFormatter 단순화 (밀리초 포함)
